...
export class VideoWidget5 extends Widget {
    ...
    setupEventListeners() {
        ...
        this.element.querySelector('#upBtn').addEventListener('click', (e) => {
            e.preventDefault();
            this.navigateWithFeedback('up', 9); // law_3
        });

        this.element.querySelector('#downBtn').addEventListener('click', (e) => {
            e.preventDefault();
            this.navigateWithFeedback('down', 8); // law_2
        });
    ...
    handleKeyDown = (event) => {
        ...
        if (key === 'w' || key === 'arrowup') {
            ...
            this.navigateWithFeedback('up', 9);
        } else if (key === 's' || key === 'arrowdown') {
            event.preventDefault();
            event.stopPropagation();
            this.navigateWithFeedback('down', 8);
        } else if (key === 'd') {
            event.preventDefault();
            event.stopPropagation();
    ...
    navigateWithFeedback(direction, targetWidget) {
        ...
        console.log(`VideoWidget5 (law_1): ${direction.toUpperCase()} → Widget ${targetWidget}`);
        ...
        this.observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && 
                    ...
                    console.log("VideoWidget5 (law_1) activated");
                }
            });
        });
    ...
    deactivate() {
        ...
        console.log('VideoWidget5 (law_1) deactivated');
    }
}
